...
const ITEM_DEFINICIONES = {
    pocion: { nombre: 'Poción', descripcion: 'Restaura 20 PS al Pokémon activo.', curar: 20, tipo: 'curar' },
    superpocion: { nombre: 'Superpoción', descripcion: 'Restaura 50 PS al Pokémon activo.', curar: 50, tipo: 'curar' },
    revivir: { nombre: 'Revivir', descripcion: 'Revive al primer Pokémon debilitado con la mitad de sus PS máximos.', tipo: 'revivir' },
    antidoto: { nombre: 'Antídoto', descripcion: 'Cura el estado de un Pokémon activo.', tipo: 'antidoto' },
};
    ...
    if (equipoJugador.length === 6) {
        ...
        if (potential.length >= 6) {
            ...
            while (equipoEnemigo.length < 6) {
                const llave = filler[i % filler.length];
                const bEnemigo = POKEDEX[llave];
                equipoEnemigo.push({
                    id: llave + '_r' + i,
                    nombre: bEnemigo.nombre,
            ...
            espejo.forEach((pk, idx) => {
                equipoEnemigo.push({
                    ...
                    hpMax: pk.hpMax,
                    hp: pk.hpMax,
                    spriteFrente: pk.spriteFrente,
                    ataques: pk.movimientos.slice(0, 2).map(m => ({ nombre: (m.nombre || '').toUpperCase(), daño: m.daño || [6,12], efecto: m.efecto })),
                    estado: null
                });
            });
        }
...
function actualizarVistaResumenEquipo() {
    ...
    while (nombres.length < 6) nombres.push("---");
...
function showItemMenu() {
    ...
    const clavesItems = Object.keys(ITEM_DEFINICIONES);
    clavesItems.forEach((clave, index) => {
        const item = ITEM_DEFINICIONES[clave];
        const cantidad = mochila[clave] || 0;
        const btn = document.createElement('div');
        btn.className = 'menu-option';
        btn.id = `item-opt-${index}`;
        btn.textContent = `${item.nombre} x${cantidad}`;
        btn.onclick = () => usarObjeto(clave);
        if (cantidad === 0) {
    ...
    mostrarStatus('Selecciona un objeto para usar.');
    actualizarEnfoqueVisualTeclado();
}

function usarObjeto(clave) {
    const item = ITEM_DEFINICIONES[clave];
    if (!item) return;
    if (!mochila[clave] || mochila[clave] <= 0) {
        mostrarStatus(`No tienes ${item.nombre}.`);
        return;
    }
    ...
    if (item.tipo === 'curar') {
        if (pk.hp <= 0) {
            mostrarStatus(`No puedes usar ${item.nombre} en un Pokémon debilitado.`);
            return;
        }
        if (pk.hp >= pk.hpMax) {
            mostrarStatus(`${pk.nombre} ya tiene la salud máxima.`);
            return;
        }

        const heal = Math.min(item.curar, pk.hpMax - pk.hp);
        pk.hp += heal;
        mochila[clave] -= 1;
        mostrarStatus(`Usaste ${item.nombre}. ${pk.nombre} recuperó ${heal} PS.`);
        actualizarHP();
        document.getElementById('battle-menu').style.display = 'none';
        menuActual = 'ninguno';
    ...
    if (item.tipo === 'revivir') {
        const debilitado = equipoJugador.find(p => p.hp <= 0);
        if (!debilitado) {
            mostrarStatus('No hay ningún Pokémon debilitado para revivir.');
            return;
        }

        const vidaRevive = Math.max(1, Math.floor(debilitado.hpMax / 2));
        debilitado.hp = vidaRevive;
        mochila[clave] -= 1;
        mostrarStatus(`Usaste ${item.nombre}. ${debilitado.nombre} revive con ${vidaRevive} PS.`);
        if (debilitado === pk) actualizarHP();
        document.getElementById('battle-menu').style.display = 'none';
        menuActual = 'ninguno';
        setTimeout(() => turnoEnemigo(), 1500);
        return;
    ...
    if (item.tipo === 'antidoto') {
        if (pk.hp <= 0) {
            mostrarStatus(`No puedes usar ${item.nombre} en un Pokémon debilitado.`);
            return;
        }
        if (!pk.estado) {
            mostrarStatus(`${pk.nombre} no tiene ningún estado alterado.`);
            return;
        }

        pk.estado = null;
        mochila[clave] -= 1;
        mostrarStatus(`Usaste ${item.nombre}. ${pk.nombre} ya no está afectado.`);
        actualizarEstadosVisuales();
        document.getElementById('battle-menu').style.display = 'none';
        menuActual = 'ninguno';
        setTimeout(() => turnoEnemigo(), 1500);
...
function actualizarEnfoqueVisualTeclado() {
    ...
    else if (menuActual === 'ataques' || menuActual === 'objetos') {
        let indexMap = (filaSeleccionada * 2) + colSeleccionada;
        let targetBtn = document.getElementById(`${menuActual === 'ataques' ? 'atk-opt' : 'item-opt'}-${indexMap}`);
...
window.addEventListener('keydown', function(event) {       
    ...
    if (tecla === 'w' || event.key === 'ArrowUp') {
        ...
        else if (menuActual === 'objetos') {
            let indexMap = (filaSeleccionada * 2) + colSeleccionada;
            let targetBtn = document.getElementById(`item-opt-${indexMap}`);
            if (targetBtn) targetBtn.click();
        }
    }
    else if (event.key === 'Escape' || event.key === 'Esc') {
        event.preventDefault();
        if (menuActual === 'ataques' || menuActual === 'objetos') {
            showActionMenu();
        } 
        else if (menuActual === 'cambio') {
